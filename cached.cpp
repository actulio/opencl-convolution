...
void parRgb2Gray(   unsigned int imgWidth,
    ...
    cl_int res = queue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(imgHeight*imgWidth));
...
void parConvolve(   unsigned int imgWidth,
    ...
    cl::CommandQueue queue(context, device, CL_QUEUE_PROFILING_ENABLE);
    cl_int res = queue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(imgWidth, imgHeight), cl::NDRange(16,16));
    queue.enqueueReadBuffer(outputImgBuf, CL_TRUE, 0, imgHeight * imgWidth * sizeof(unsigned char ), outputImg);
...
void initializeDevice(){
    ...
    std::ifstream kernel_file("cached.cl");
...
bool checkEquality(unsigned char* img1,
    ...
    if(errors > 0 ){
        printf("erros=%.0f total=%.0f porcentagem=%.2f\n",errors, all, (float) (errors/all) * 100 );
        return false;
    }
